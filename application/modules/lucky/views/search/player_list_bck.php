			...
			"columnDefs": [
				...
				"render": function ( data, type, row, meta ) {
					if( row[7] == 0 ) { // status == 1 active
						var status_html =  '<span id="up_id'+row[6]+'"><i class="fa fa-lock"  onclick="status('+row[7]+','+row[6]+');" aria-hidden="true"></i></span>';
					} else { // status == 2 deactive
						var status_html =  '<span id="up_id'+row[6]+'"><i class="fa fa-unlock" onclick="status('+row[7]+','+row[6]+');" aria-hidden="true"></i></span>';
					}
					return '<span style="cursor:pointer;" id="search_partner'+row[6]+'">'+ status_html +'&nbsp;&nbsp;<i class="fa fa-info-circle" onclick="usersetting('+row[7]+');" aria-hidden="true"></i>&nbsp;&nbsp;</span>'
				} }, 
